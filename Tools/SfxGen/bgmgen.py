...
**주의: 이 파일은 더 이상 배포 음원을 만들지 않는다.** Assets/Audio/Bgm의
bgm_*.wav는 전부 bgmgen_snes.py 산출물이다(스테레오 32kHz — 이쪽은 모노
44.1kHz라 파일 헤더만 봐도 구분된다). 여기를 고쳐도 게임 음악은 바뀌지 않는다.
NES 음색 시절의 원본으로만 남겨 둔다 (화성 설계는 bgmgen_snes가 물려받았다).
...
THEME_PRESETS = {
    ...
    "boss": {
        "bpm": 172,
        "progression": [-24, -24, -18, -19, -24, -24, -17, -12],  # 반복 압박 + 5도 도약 (전투)
        "scale": [0, 2, 3, 5, 7, 8, 11, 12],  # 하모닉 마이너 (긴박)
        "arp": [12, 15, 19, 24, 27, 24, 19, 15],
    },
    "title": {
        "bpm": 96,
        "progression": [-24, -19, -16, -12, -24, -19, -14, -12],  # 상승 희망 진행 (오프닝)
        "scale": [0, 2, 4, 7, 9, 12],         # 메이저 펜타토닉 (서정)
        "arp": [12, 16, 19, 24, 28, 24, 19, 16],
    },
}
