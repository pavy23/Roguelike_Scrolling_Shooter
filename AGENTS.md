...
- 에이전트는 **자기 브랜치에만 커밋한다.** (이건 그대로다 — 소유 경계와 같은 규칙이다.)
- **병합은 오케스트레이터가 수행한다** (2026-08-03 사람 지시로 변경, 이전에는 사람 전용).
  절차: (1) `git log --oneline main..<브랜치>`로 들어올 커밋 확인, (2) 겹치는 파일 확인
  (`comm -12`로 양쪽 변경 파일 교집합 — 비어 있으면 충돌 없음), (3) `--no-ff`로 병합해
  어느 브랜치에서 왔는지 이력에 남긴다, (4) 병합 후 **양쪽 검증을 다시 돌린다**
  (`dotnet test` + `unity test --mode EditMode`) — 각자 브랜치에서 통과해도 합친 뒤
  깨질 수 있다, (5) 통과해야 다음 작업을 얹는다.
- 충돌이 나거나 밸런스 수치(§7)가 걸린 병합은 **멈추고 사람에게 묻는다.**
...
### 9-1. 담당 에이전트를 못 쓸 때의 대체 (2026-08-03 사람 지시)

토큰 소진·연결 실패로 담당을 부를 수 없으면 **작업을 멈추지 말고 대체자가 이어받는다.**
판단은 오케스트레이터가 하고, 무엇을 왜 옮겼는지 커밋 메시지에 남긴다.

| 원 담당 | 1순위 대체 | 근거 |
|---|---|---|
| **CODEX** (Core) | **CLAUDE** | 결정론 규칙(§4)이 걸린 C# 작업이다. 규칙을 이미 지고 있고 세션 맥락이 있는 쪽이 사고를 덜 낸다. GROK은 데이터 담당이라 Core 관례가 없다 |
| **GROK** (데이터) | **CLAUDE** | JSON 수치 조정. 다만 **밸런스 기본값 확정은 §7대로 사람 몫**이라, 대체자는 계산·제안까지만 하고 확정은 미룬다 |
| **GEMINI** (QA) | **CLAUDE** | 이미 `Tools/QaHarness`로 headless 검증을 돌리고 있다 |
| **CLAUDE** (Presentation) | **CODEX** | 렌더러가 막히면 Core 담당이 대신. 아트 생성은 대체 불가 — 사람 판단 대기 |

**대체 시 지켜야 할 것**
1. **소유 경계를 넘는다는 사실을 커밋에 명시**한다 — "CODEX 부재로 CLAUDE가 대행" 식으로.
   나중에 원 담당이 돌아왔을 때 무엇이 남의 손을 탔는지 보여야 한다.
2. 검증 기준은 **원 담당의 것을 그대로 쓴다** (Core면 `dotnet test`, 데이터면 스키마 검증).
3. §7(밸런스 수치)과 아트 채택은 대체로 넘어가지 않는다 — 사람 결정 사안이다.
4. 원 담당이 복귀하면 대행분을 **리뷰 대상으로 넘긴다** (`Reviews/from-claude/requests.md`에 한 줄).
