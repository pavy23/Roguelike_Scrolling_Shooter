...
진행은 **각자의 브라우저에** 남는다. 크레딧·기체 해금·컨티뉴 재고·난이도·접근성
설정은 플레이한 기기에 저장되고 빌드에는 들어가지 않는다 — 남에게 복사해 줘도
상대는 0에서 시작한다. 그 페이지의 사이트 데이터를 지우면 함께 사라진다.

공유되는 것은 **글로벌 스코어보드** 하나뿐이고, 그것도 직접 제출할 때만이다 —
파일럿 이름·점수·런 요약만 보낸다.
