...
namespace Shmup.Core.Tests
{
    ...
    public sealed class Req177WarshipPhase2BurstTests
    {
        ...
        /// 그 틈을 탄으로 다시 메우면 회피 경로가 사라진다.
        ///
        /// 처음에는 8.0으로 잡았다. 사람이 2026-08-05에 그 선을 두 번 직접 옮겼다 —
        /// 먼저 "상한 8발/초 최대치를 맞춰줘"로 거기에 붙였고, 화면에서 확인한 뒤
        /// "모든 탄막 주기만 지금의 1/2로 하자"로 다시 두 배를 요구했다. 즉 이
        /// 상한은 사람이 화면을 보고 정하는 값이고, 여기 남은 역할은 **의도치 않은
        /// 자릿수 변화를 잡는 것**이다. 16이면 6문이 평균 0.4초에 한 번씩 내는
        /// 셈이다 — 그보다 위는 사람이 고른 밀도가 아니라 사고다.
        /// </summary>
        const double MaxCombinedBulletsPerSecond = 16.0;
