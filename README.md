...
- **미지의 구역에는 화면 절반을 채우는 것이 둘 있다.** 정체는 여기 적지 않는다 — 조건 두 개를 채워야 열리는 것이라, 미리 알려 주면 도착했을 때의 보상이 사라진다. 하나는 외갑을 뚫어야 속이 열리고, 다른 하나는 아가리가 벌어질 때가 가장 위험하다. 둘 다 마지막에 **몸을 버리고 다시 나온다**.
...
- **보스마다 곡이 다르다.** 중간보스는 전투곡(178bpm) 그대로지만, 스테이지 보스는 112bpm으로 무게를 싣고, 히든 보스는 76bpm에 어디에도 착지하지 않는 화성으로 내려앉는다. 셋의 간격이 넓어야 "다른 판에 왔다"가 귀로 읽힌다 — 한때 스테이지 보스가 92bpm이었는데 히든과 16bpm 차이뿐이라 두 곡이 한 곡으로 들렸다.
